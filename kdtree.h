...
class KDTree
{
    ...
    private:
         ...
         */
        int sqdis(const Point<Dim> & target, const Point<Dim> & p) const;
        int partition(int left, int right,int curDim,int pivotIndex);
        void partall(int left, int right, int curDim,int n);
        void sort(int left, int right, int curDim);
        int dimdis(const Point<Dim> & target, const Point<Dim> & p, int curDim) const;
        Point<Dim> nearest(const Point<Dim>&query, int left, int right, int curDim) const;
};
